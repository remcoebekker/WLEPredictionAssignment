...
- Random forest seems to produce a better accuracy than KNN so I will use the random forest model to predict the 20 cases in the validation set
...
```{r, fig.width = 5, fig.height = 7, cache = TRUE}
## We train the model
rfModel <- train(classe ~ ., data = trainingData, ntree = 125, method = "rf", trControl = trainControl(method = "cv", number = 10, verboseIter = FALSE), prox = TRUE)
...
## Out of sample error
Now that we have trained the model on the training data, we can determine the out of sample error by applying the model to the test data set and creating a confusion matrix to see the accuracy related statistics.
```{r}
